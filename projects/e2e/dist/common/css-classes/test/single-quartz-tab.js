...
export const testSingleQuartzTab = (code, contentSelector, schema, tabLink, type, exact = false) => {
    const { findInContainer } = getSharedHelper(code);
    const find = (selector) => findInContainer(`${contentSelector}[tab-name="${type}"] ${selector}`);
    describe(type, () => {
